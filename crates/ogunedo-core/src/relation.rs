...
use crate::{
    derive_matrix_polynomial, negacyclic_mul, parameters, relation_digest, statement_digest,
    ParameterStatus, Parameters, PublicStatement, PublicValues, Witness, PROTOCOL_VERSION,
};
...
pub enum RelationError {
    ...
    NonCanonicalTarget { index: usize, value: u32 },
    WitnessLength { expected: usize, actual: usize },
    CoefficientOutOfRange { index: usize, value: i32, bound: i32 },
    L2BoundExceeded { actual: u64, maximum: u64 },
    EquationMismatch { index: usize, expected: u32, actual: u32 },
}
...
impl fmt::Display for RelationError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ParameterArithmeticInvalid => write!(formatter, "parameter arithmetic is invalid"),
            ...
            Self::NonCanonicalTarget { index, value } => {
                write!(formatter, "target coefficient {index} is non-canonical: {value}")
            }
            Self::WitnessLength { expected, actual } => {
                write!(formatter, "witness length {actual}, expected {expected}")
            }
            Self::CoefficientOutOfRange { index, value, bound } => write!(
                formatter,
            ...
            Self::EquationMismatch { index, expected, actual } => write!(
                formatter,
...
fn validate_parameters(parameters: Parameters) -> Result<(), RelationError> {
    if !parameters.ring_degree.is_power_of_two()
        ...
        || ((parameters.q - 1) as usize) % (2 * parameters.ring_degree) != 0
    ...
    for row in 0..parameters.rows {
        ...
        for column in 0..parameters.columns {
            let matrix_polynomial =
                derive_matrix_polynomial(matrix_seed, row, column, parameters);
...
pub fn verify_relation(
    ...
    if statement.protocol_version != PROTOCOL_VERSION {
        return Err(RelationError::UnsupportedProtocolVersion(statement.protocol_version));
    }
    let selected =
        parameters(statement.parameter_id).ok_or(RelationError::UnknownParameterSet(statement.parameter_id))?;
    ...
    for (index, &coefficient) in statement.target.iter().enumerate() {
        if coefficient >= selected.q {
            return Err(RelationError::NonCanonicalTarget { index, value: coefficient });
        }
    }
    ...
    for (index, (&expected, &actual)) in statement.target.iter().zip(&computed).enumerate() {
        if expected != actual {
            return Err(RelationError::EquationMismatch { index, expected, actual });
        }
    }
    ...
    Ok(VerificationReceipt { parameters: selected, public_values })
}
